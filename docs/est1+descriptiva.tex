...
\date{Actualizado al 02-02-2023}
...
Una vez obtenido el gráfico, podemos comparar las dispersiones de los datos en cada uno de los años. En específico, se aprecia que la dispersión se encuentra principalmente entre 1.1 y 1.3 en la mayoría de los años, a excepción del periodo 2007 - 2014, el cual, refleja valores superiores a este rango.
...
\tightlist
\item
  Utilizando las variables \textbf{High} y \textbf{Open} de la base de datos de la unidad, elabore un gráfico de barras para cada una, para ello, establezca 2 puntos de cortes (3 categorías) a su elección. Interprete.
\end{enumerate}
...
\NormalTok{g }\OtherTok{=} \FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data =}\NormalTok{ datos[datos}\SpecialCharTok{$}\NormalTok{Ano }\SpecialCharTok{\%in\%} \DecValTok{2020}\SpecialCharTok{:}\DecValTok{2022}\NormalTok{,],}
  ...
  \FunctionTok{facet\_wrap}\NormalTok{(}\FunctionTok{vars}\NormalTok{(Ano), }\AttributeTok{nrow =} \DecValTok{1}\NormalTok{, }\AttributeTok{scales =} \StringTok{"free\_x"}\NormalTok{)}
\NormalTok{g}
\end{Highlighting}
\end{Shaded}
